...
class MetadataModel(Model):
    ...
    def new_plugin_version(cls, metadata, plugin_id, filename):
        ...
        try:
            ...
            get_log().error("PluginNotFound", requestId=g.request_id, pluginId=plugin_id)
            raise DataError(400, "Plugin Not Found")
        # Update version zero
        cls.update_version_zero(metadata, version_zero, filename)
        get_log().info("VersionZeroUpdated", requestId=g.request_id, pluginId=plugin_id)

        # Insert former v0 into revision
        cls.revision_former_version_zero(version_zero.attribute_values)
        version_zero.refresh()
        get_log().info("RevisionInserted", requestId=g.request_id, pluginId=plugin_id, revision=version_zero.revisions)

        updated_metadata = json.loads(json.dumps(version_zero.attribute_values, cls=ModelEncoder))
        get_log().info("MetadataStored", requestId=g.request_id, metadata=updated_metadata)
    ...
    def validate_token(cls, token, plugin_id):
        ...
        if token != metadata.secret:
            get_log().error("InvalidToken", requestId=g.request_id, pluginId=plugin_id)
